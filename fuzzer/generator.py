...
Входные данные:
- Ключи "input_data" ДОЛЖНЫ в точности соответствовать именам параметров функции. НЕ добавляй дополнительные параметры. НЕ пропускай обязательные. Для позиционных параметров используй их имена как ключи.
...
- Чтобы проверить state leakage / повторные вызовы, используй формат последовательности: "input_data" — массив входов, "expected" — массив ожиданий (по одному на вызов).
