...
- expected.value для математической операции — результат ФАКТИЧЕСКОГО вычисления по контракту. Если результат выходит за пределы float (переполнение) и Python вернёт inf — используй специальный маркер: {{"type": "return", "value": "inf"}} (для отрицательного переполнения — "value": "-inf"). Раннер сравнит его с math.isinf(actual). НЕ подменяй переполняющий результат произвольным конечным числом: 1e308 / 1e-308 = 1e616 — это НЕ 1e308 и не представимо в float, ожидание должно быть "inf".
...
- результат превышает диапазон float (переполнение, Python вернёт inf): {{"type": "return", "value": "inf"}} (для отрицательного переполнения — "value": "-inf")
